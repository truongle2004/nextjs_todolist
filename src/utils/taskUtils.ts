...
export const calculateTodoProgress = (tasks: Task[]): {
  completed: number;
  total: number;
  percentage: number;
} => {
  const total = tasks.length;
  const completed = tasks.filter(task => task.completed || task.status === StatusEnum.DONE).length;
...
export const filterTasksByStatus = (tasks: Task[], status?: StatusEnum): Task[] => {
  if (!status) return tasks;
  return tasks.filter(task => task.status === status);
};

export const filterTasksByPriority = (tasks: Task[], priority?: TaskPriorityEnum): Task[] => {
...
export const filterDueSoonTasks = (tasks: Task[], daysAhead: number = 3): Task[] => {
  return tasks.filter(task => 
    task.due_date && 
    !task.completed && 
    task.status !== StatusEnum.DONE &&
